...
namespace Hash_bucket
{
	...
	struct _Hash_Iterator
	{
		...
		typedef Hash_List_Node<Data>* PNode;

		typedef const Hash<Key, Data, Fetch_Ruler, Transform_Ruler>* PHash;

		_Hash_Iterator(size_t n, PHash ph, PNode pn) :
			_num(n),
			_pHash(ph),
			_pNode(pn)
		{
		...
		Self& operator++()
		{
			...
			{
				while (!(_pHash->_table[++_num])) {}
				_pNode = _pHash->_table[_num];
			}
			return *this;
		}
		...
		{
			return (_num != x._num) || (_pHash != x._pHash) || (_pNode != x._pNode);
		}
	...
	public:
		...
		iterator begin()
		{
			...
			size_t cur = 0;
			while (!_table[++cur]) {}
			return iterator(cur, this, _table[cur]);
		}
		...
		const_iterator begin() const
		{
			...
			size_t cur = 0;
			while (!_table[++cur]) {}
			return const_iterator(cur, this, _table[cur]);
		}

		iterator end()
		{
			return iterator(_table.size(), this, nullptr);
		}

		const_iterator end() const
		{
			return const_iterator(_table.size(), this, nullptr);
		}
		...
		bool insert(const Data& d)
		{
			...
			while (tmp)
			{
				if (tr(fr(tmp->_data)) == tr(fr(d))) return false;
				tmp = tmp->_next;
			}
